...
工具转换的一个强大用例是将现有 Python 客户端（GitHub 客户端、API 客户端、数据库客户端等）的方法直接公开为 MCP 工具。这种模式消除了样板代码包装函数，并将工具视为客户端方法周围的注释。

**不使用工具转换**，您通常会创建重复注释的包装函数：
...
mcp = FastMCP("GitHub 工具")

# 直接将客户端方法包装为工具
get_repo_tool = Tool.from_tool(
    tool=Tool.from_function(fn=github_client.get_repository),
    description="获取关于 GitHub 仓库的基本信息。",
    transform_args={
        "owner": ArgTransform(description="仓库所有者。"),
        "repo": ArgTransform(description="仓库名称。"),
    }
)
...
这种模式将实现保留在您的客户端中，并将工具视为注释层，避免重复代码。

#### 隐藏客户端特定参数

客户端方法通常具有不应向 LLM 公开的内部参数（调试标志、身份验证令牌、速率限制设置）。使用 `hide=True` 和默认值来自动处理这些参数：

```python
get_issues_tool = Tool.from_tool(
    tool=Tool.from_function(fn=github_client.get_issues),
    description="从 GitHub 仓库获取问题。",
    transform_args={
        "owner": ArgTransform(description="仓库所有者。"),
        "repo": ArgTransform(description="仓库名称。"),
        "limit": ArgTransform(description="返回的最大问题数。"),
        # 隐藏内部参数
...
LLM 只看到 `owner`、`repo` 和 `limit`。内部参数自动提供。

#### 可重用的参数模式

在包装多个客户端方法时，您可以定义可重用的参数转换。这对于大型工具集具有良好的可扩展性，并保持注释的一致性：
...
# 定义可重用的参数模式
OWNER_ARG = ArgTransform(description="仓库所有者。")
REPO_ARG = ArgTransform(description="仓库名称。")
LIMIT_ARG = ArgTransform(description="返回的最大项目数。")
HIDE_ERROR = ArgTransform(hide=True, default=True)

def create_github_tools(client):
    """使用共享参数模式从 GitHub 客户端方法创建工具。"""
    ...
    return [
        ...
        Tool.from_tool(
            ...
            description="从仓库获取特定问题。",
            transform_args={
                **owner_repo_args,
                "issue_number": ArgTransform(description="问题编号。"),
        ...
        Tool.from_tool(
            ...
            description="从仓库获取特定拉取请求。",
            transform_args={
                **owner_repo_args,
                "pull_request_number": ArgTransform(description="PR 编号。"),
                "limit_comments": LIMIT_ARG,
                **error_args,
            }
        ),
