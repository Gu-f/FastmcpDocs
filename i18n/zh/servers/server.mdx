...
<ParamField body="version" type="str | None">
  服务器的版本字符串。如果未提供，默认为 FastMCP 库版本
</ParamField>

<ParamField body="website_url" type="str | None">
  <VersionBadge version="2.14.0" />

  包含有关服务器更多信息的网站 URL。在客户端应用程序中显示
</ParamField>

<ParamField body="icons" type="list[Icon] | None">
  <VersionBadge version="2.14.0" />

  服务器的图标表示列表。图标帮助用户在客户端应用程序中视觉识别您的服务器。详细示例请参阅[图标](/zh/servers/icons)
</ParamField>
...
<ParamField body="strict_input_validation" type="bool" default="False">
  ...
  控制如何验证工具输入参数。当为 `False`（默认）时，FastMCP 使用 Pydantic 的灵活验证，强制转换兼容的输入（例如，`"10"` → `10` 用于整数参数）。当为 `True` 时，使用 MCP SDK 的 JSON Schema 验证在将输入传递给函数之前根据确切架构验证输入，拒绝任何类型不匹配。默认模式提高了与 LLM 客户端的兼容性，同时保持类型安全。详细信息请参阅[输入验证模式](/zh/servers/tools#input-validation-modes)
</ParamField>
...
FastMCP 支持几种传输选项：
- STDIO（默认，用于本地工具）
- Streamable HTTP（推荐用于 Web 服务，使用Streamable HTTP 协议）
- SSE（传统 Web 传输，已弃用）
...
对于更复杂的 Web 应用程序，请考虑[将您的 MCP 服务器挂载到 FastAPI 或 Starlette 应用程序中](/zh/deployment/http#integration-with-web-frameworks)。
...
print(fastmcp.settings.strict_input_validation)  # 默认：False
...
- **`strict_input_validation`**：控制工具输入验证模式（默认：False 进行灵活强制转换），使用 `FASTMCP_STRICT_INPUT_VALIDATION` 设置。详细信息请参阅[输入验证模式](/zh/servers/tools#input-validation-modes)
- **`include_fastmcp_meta`**：是否在组件响应中包含 FastMCP 元数据（默认：True），使用 `FASTMCP_INCLUDE_FASTMCP_META` 设置
- **`env_file`**：要从中加载设置的环境文件路径（默认：".env"），使用 `FASTMCP_ENV_FILE` 设置。当您的项目使用语法与 python-dotenv 不兼容的 `.env` 文件时很有用
