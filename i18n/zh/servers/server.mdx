...
每种组件类型都有详细文档：[工具](/zh/servers/tools)、[资源](/zh/servers/resources)（包括[资源模板](/zh/servers/resources#资源模板)）和[提示词](/zh/servers/prompts)。
...
也可以使用 FastMCP CLI 运行服务端。有关传输和部署的详细信息，请参阅[运行服务端](/zh/deployment/running-server)。
...
<ParamField body="icons" type="list[Icon] | None">
  ...
  服务端图标表示列表。详情请参阅[图标](/zh/servers/icons)
</ParamField>
...
<ParamField body="auth" type="OAuthProvider | TokenVerifier | None">
  用于保护基于 HTTP 的传输的身份认证 provider。配置请参阅[身份认证](/zh/servers/auth/authentication)
</ParamField>

<ParamField body="middleware" type="list[Middleware] | None">
  [中间件](/zh/servers/middleware)会拦截并转换流经服务端的每条 MCP 消息，包括双向的 requests、responses 和 notifications。适用于日志、错误处理、限流等横切关注点
</ParamField>

<ParamField body="providers" type="list[Provider] | None">
  [Providers](/zh/servers/providers) 用于动态提供工具、资源和提示词。Providers 会在请求时被查询，因此可以从数据库、API 或其他外部来源提供组件
</ParamField>

<ParamField body="transforms" type="list[Transform] | None">
  <VersionBadge version="3.1.0" />

  应用于所有组件的服务端级 [transforms](/zh/servers/transforms/transforms)。Transforms 会修改工具、资源和提示词呈现给客户端的方式；例如，[搜索 transforms](/zh/servers/transforms/tool-search) 可以用按需发现替代大型目录
</ParamField>

<ParamField body="lifespan" type="Lifespan | AsyncContextManager | None">
  服务端启动和停止时运行的服务端级 setup 与 teardown 逻辑。有关可组合 lifespans，请参阅 [Lifespans](/zh/servers/lifespan)
</ParamField>
</Card>
...
<ParamField body="strict_input_validation" type="bool" default="False">
  ...
  当为 `False`（默认）时，FastMCP 使用 Pydantic 的灵活校验，会强制转换兼容输入（例如 int 参数的 `"10"` → `10`）。当为 `True` 时，会在调用函数前根据精确 JSON Schema 校验输入，并拒绝类型不匹配。详情请参阅[输入校验模式](/zh/servers/tools#验证模式)
</ParamField>
...
<ParamField body="list_page_size" type="int | None" default="None">
  ...
  list 操作（`tools/list`、`resources/list` 等）每页的最大条目数。当为 `None` 时，所有结果会在单个响应中返回。详情请参阅[分页](/zh/servers/pagination)
</ParamField>
...
<ParamField body="sampling_handler" type="SamplingHandler | None">
  MCP 采样请求（服务端发起的 LLM 调用）的自定义处理器。详情请参阅[采样](/zh/servers/sampling)
</ParamField>
...
自定义路由适用于健康检查、状态 endpoints 和简单 webhooks。对于更复杂的 Web 应用，可以考虑[将 MCP 服务端挂载到 FastAPI 或 Starlette app 中](/zh/deployment/http#与-web-框架集成)。
