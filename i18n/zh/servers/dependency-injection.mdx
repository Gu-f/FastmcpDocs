...
title: 依赖注入
sidebarTitle: 依赖
description: 将 HTTP 请求、访问令牌和自定义依赖等运行时值注入 MCP 组件。
icon: syringe
tag: NEW
---
...
The MCP Context provides logging, progress reporting, resource access, and other request-scoped operations. See [MCP Context](/zh/servers/context) for the full API.
