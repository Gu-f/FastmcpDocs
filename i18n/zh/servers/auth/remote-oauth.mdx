...
| **FastMCP 类** | `RemoteAuthProvider` | [`OAuthProxy`](/servers/auth/oauth-proxy) |

如果您的提供商不支持 DCR（大多数传统的 OAuth 提供商），您需要使用 [`OAuth 代理`](/servers/auth/oauth-proxy) 代替，它弥合了 MCP 的 DCR 期望与固定 OAuth 凭据之间的差距。
...
mcp = FastMCP(name="公司 API", auth=auth)
```
...
`authorization_servers` 列表告诉 MCP 客户端您信任哪些身份提供商。`resource_server_url` 在 OAuth 元数据中标识您的服务器，启用适当的令牌受众验证。
