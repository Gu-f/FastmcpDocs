...
远程 OAuth 集成允许您的 FastMCP 服务器利用外部身份提供商，同时保持 MCP 客户端所需的自动化身份验证流程。这种方法提供企业级身份验证功能，而无需自己实现复杂性，使其成为大多数生产应用程序的推荐模式。

<Tip>
远程 OAuth 需要支持**动态客户端注册（DCR）**的身份提供商。这使得 MCP 客户端能够自动注册和验证身份，无需手动配置步骤。
</Tip>
