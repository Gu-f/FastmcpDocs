...
description: 将您的FastMCP服务器与Descope、WorkOS、Auth0和公司SSO系统等外部身份提供程序集成。
icon: camera-cctv
tag: NEW
---
...
- **RemoteAuthProvider**：适用于具有动态客户端注册的提供商（Descope、WorkOS AuthKit、现代OIDC提供商）
- **OAuth Proxy**: 适用于没有动态客户端注册的提供商（GitHub、Google、Azure、AWS、Discord等）
...
| **配置** | 零客户端配置 | 预共享凭据 |
| **示例** | Descope, WorkOS AuthKit, modern OIDC | GitHub, Google, Azure |
...
auth = RemoteAuthProvider(
    ...
    base_url="https://api.yourcompany.com",  # 您的服务器基础 URL
    # 可选：自定义允许的客户端重定向 URI（默认仅为 localhost）
...
`authorization_servers` 列表告诉 MCP 客户端您信任哪些身份提供程序。`base_url` 在 OAuth 元数据中标识您的服务器，启用正确的令牌受众验证。**重要**：`base_url` 应该指向您的服务器基础 URL - 例如，如果您的 MCP 服务器可以通过 `https://api.yourcompany.com/mcp` 访问，请使用 `https://api.yourcompany.com` 作为基础 URL。
...
<Note>
`RemoteAuthProvider` 还支持 `allowed_client_redirect_uris` 参数，用于控制在 DCR 期间从 MCP 客户端接受哪些重定向 URI：

- `None`（默认）：仅允许 localhost 模式
- 自定义列表：指定支持通配符的允许模式
- 空列表 `[]`：允许全部（不推荐）

这提供了深度防御，尽管 DCR 提供程序通常会自己验证重定向 URI。
</Note>
