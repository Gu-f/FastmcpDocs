...
FastMCP 包含针对流行 OAuth 服务的预配置提供商，可通过单个环境变量激活：

<ParamField path="FASTMCP_SERVER_AUTH" type="string">
要使用的身份验证提供商。支持的值：
- `GITHUB` - GitHub OAuth（需要额外的 GitHub 特定环境变量）
- `GOOGLE` - Google OAuth（需要额外的 Google 特定环境变量）
- `JWT` - JWT 令牌验证
- `WORKOS` - WorkOS AuthKit
- 自定义提供商类名
</ParamField>

使用 GitHub 或 Google 等注册提供商时，您需要设置提供商特定的环境变量：
