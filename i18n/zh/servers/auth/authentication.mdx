...
**对于不支持 DCR 的 OAuth 提供程序（GitHub、Google、Azure、大多数企业系统），请使用 OAuth 代理。** 这些提供程序需要通过其开发者控制台手动注册应用程序。OAuth 代理通过向 MCP 客户端提供符合 DCR 的接口同时使用您与提供程序的固定凭据来弥合差距。代理的回调转发模式使动态客户端端口能够与需要固定重定向 URI 的提供程序一起工作。

**对于支持 DCR 的身份提供程序（WorkOS AuthKit、现代认证平台），请使用 RemoteAuthProvider。** 这些提供程序允许客户端动态注册并获取凭据而无需手动配置。这启用了 MCP 设计的完全自动化身份验证流程，提供最佳用户体验和最简单的实现。
