...
<ParamField body="token_verifier" type="TokenVerifier" required>
  A [`TokenVerifier`](/zh/servers/auth/token-verification) instance to validate the provider's tokens
</ParamField>
...
See the [Token Verification guide](/zh/servers/auth/token-verification) for detailed setup instructions for your provider.
