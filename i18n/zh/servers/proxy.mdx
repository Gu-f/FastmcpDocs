...
If low latency is a requirement for your use-case, consider using [`import_server()`](/zh/servers/composition#importing-static-composition) to copy tools at startup rather than proxying them at runtime.
...
但是，您可以创建镜像组件的副本并将其存储为新的本地定义组件。本地组件始终优先于镜像组件，因为代理服务器会在尝试接触远程服务器之前检查自己的注册表。
