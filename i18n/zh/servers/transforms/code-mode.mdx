...
title: 代码模式
sidebarTitle: 代码模式
description: 让 LLM 在沙箱中编写 Python 来编排工具
icon: flask
tag: NEW
---
...
If your tools use [tags](/zh/servers/tools), Search also accepts a `tags` parameter so the LLM can narrow results to specific categories before searching.
...
`GetTags` lets the LLM browse tools by category using [tag](/zh/servers/tools) metadata. At brief detail, the LLM sees tag names with counts. At full detail, it sees tools listed under each tag:
...
If your tools use [tags](/zh/servers/tools), add `GetTags` so the LLM can browse by category before searching — giving it four stages of progressive disclosure:
