...
原始工具仍然可以调用。它们会从列表中隐藏，但仍保持完整功能；搜索转换控制的是*发现*，不是*访问*。
...
`BM25SearchTransform` 使用 [BM25 Okapi](https://en.wikipedia.org/wiki/Okapi_BM25) 算法按相关性对工具排序。它更适合自然语言查询，因为它会基于词频和词项稀有度给每个工具打分，并按相关性排序返回，而不是简单地按“匹配/不匹配”过滤。
