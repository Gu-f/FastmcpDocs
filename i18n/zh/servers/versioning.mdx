...
The `remove_tool`, `remove_resource`, and `remove_prompt` methods on the server's [local provider](/zh/servers/providers/local) accept an optional `version` parameter that controls what gets removed.
