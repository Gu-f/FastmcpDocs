...
sidebarTitle: Local
description: 通过装饰器注册组件时使用的默认 provider
icon: house
tag: NEW
---
...
关于 keys、tags、allowlist 模式和 provider 级控制的完整文档，请参阅[可见性](/zh/servers/visibility)。
...
独立 provider 也支持使用 `enable()` 和 `disable()` 进行可见性控制。详情请参阅[可见性](/zh/servers/visibility)。
