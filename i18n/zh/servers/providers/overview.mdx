...
title: Providers
sidebarTitle: 概览
description: FastMCP 如何获取工具、资源和提示词
icon: layer-group
tag: NEW
---
...
[Transforms](/zh/servers/transforms/transforms) 会在组件从 providers 流向客户端时修改组件。每个 transform 都位于一条链中，拦截查询并在继续传递前修改结果。
...
transforms 可以添加到单个 provider（只影响该来源），也可以添加到服务端本身（影响所有组件）。完整说明请参阅 [Transforms](/zh/servers/transforms/transforms)。
...
- 将[另一个服务端挂载](/zh/servers/composition)到你的服务端中
- 通过你的服务端[代理远程服务端](/zh/servers/providers/proxy)
- [控制组件可见性状态](/zh/servers/visibility)
- [构建动态来源](/zh/servers/providers/custom)，例如数据库驱动的工具

## 下一步

- [Local](/zh/servers/providers/local) - 装饰器如何工作
- [挂载](/zh/servers/composition) - 将服务端组合在一起
- [代理](/zh/servers/providers/proxy) - 连接到远程服务端
- [Transforms](/zh/servers/transforms/transforms) - 为组件添加命名空间、重命名和修改组件
- [可见性](/zh/servers/visibility) - 控制客户端可以访问哪些组件
- [自定义](/zh/servers/providers/custom) - 构建你自己的 providers
