...
两个参数都是生产所必需的。**将您的存储包装在 `FernetEncryptionWrapper` 中以加密静态的敏感 OAuth 令牌** - 没有它，令牌将以明文存储。有关完整的设置详情，请参阅 [OAuth 令牌安全](/zh/deployment/http#oauth-令牌安全性) 和 [密钥和存储管理](/zh/servers/auth/oauth-proxy#密钥和存储管理)。

### 响应缓存中间件

[响应缓存中间件](/zh/servers/middleware#缓存中间件) 缓存工具调用、资源读取和提示请求。存储配置通过 `cache_storage` 参数传递：
...
- [响应缓存中间件](/zh/servers/middleware#缓存中间件) - 使用存储进行缓存
- [OAuth 令牌安全](/zh/deployment/http#oauth-令牌安全性) - 生产 OAuth 配置
