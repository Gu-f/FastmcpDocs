...
<Tip>
请注意：FastMCP 对通配符参数的支持是模型上下文协议标准的**扩展**，该标准在其他方面遵循 RFC 6570。由于所有模板处理都在 FastMCP 服务器中进行，这不应该与其他 MCP 实现产生任何兼容性问题。
</Tip>
...
def search_resources(query: str, max_results: int = 10, include_archived: bool = False) -> dict:
    """搜索与查询字符串匹配的资源。"""
    # 只有 'query' 在 URI 中是必需的，其他参数使用它们的默认值
...
使用此模板，客户端可以请求 `search://python`，函数将以 `query="python", max_results=10, include_archived=False` 被调用。MCP 开发人员仍可以使用更具体的参数直接调用底层的 `search_resources` 函数。

您还可以通过手动将装饰器应用于单个函数来创建多个资源模板，以提供访问相同底层数据的不同方式：
