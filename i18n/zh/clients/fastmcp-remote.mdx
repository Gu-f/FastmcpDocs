...
description: 使用 uvx fastmcp-remote 将远程 MCP 服务端桥接到仅支持 stdio 的 MCP host。
icon: bridge
---
...
该包由 FastMCP 驱动。它会为远程 URL 构建一个 FastMCP 客户端，将该客户端暴露为本地 stdio 代理，并让可执行文件专注于这层桥接。若要运行 Python 服务端文件、本地项目环境、FastMCP 配置文件和开发重载循环，请使用 [`fastmcp run`](/zh/cli/running)。

命令形式沿用了最初的 [`mcp-remote`](https://github.com/geelen/mcp-remote) npm 项目，该项目为 MCP host 建立了这种 stdio 到远程服务端的桥接模式。
