...
- **`token_storage`** (`AsyncKeyValue`，可选)：用于持久化 OAuth 令牌的存储后端。默认使用内存存储（重启后令牌会丢失）。参阅[令牌存储](#令牌存储)了解加密存储选项
...
<Steps>
<Step title="令牌检查">
客户端首先检查已配置的 `token_storage` 后端中是否存在目标服务器的有效令牌。如果找到，它将用于验证客户端。
</Step>
...
<Step title="令牌缓存">
获得的令牌会保存到已配置的 `token_storage` 后端，以便后续使用，从而避免重复的浏览器交互。
</Step> 
<Step title="已验证请求">
...
默认情况下，令牌存放在内存中，应用一旦重启就会丢失。若需持久化存储，请将兼容 `AsyncKeyValue` 的存储后端传入 `token_storage` 参数。

<Warning>
**安全注意事项**：生产环境请使用加密存储。MCP 客户端可能随着时间积累多个服务器的 OAuth 凭据，若令牌存储被入侵，可能会导致多个服务受影响。
</Warning>
...
你可以使用 [key-value 库](https://github.com/jlowin/key-value) 中任意兼容 `AsyncKeyValue` 的后端，例如 Redis、DynamoDB 等。请使用 `FernetEncryptionWrapper` 为存储添加加密保护。
