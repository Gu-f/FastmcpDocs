...
- **`client_id`** (`str`，可选)：预注册的 OAuth 客户端 ID。提供后会完全跳过 Dynamic Client Registration。参见[预注册客户端](#预注册客户端)
- **`client_secret`** (`str`，可选)：预注册客户端的 OAuth client secret。可选，依赖 PKCE 的公开客户端可以省略
- **`client_metadata_url`** (`str`，可选)：基于 URL 的客户端身份 (CIMD)。详见 [CIMD 认证](/zh/clients/auth/cimd)
...
关于创建、托管和验证 CIMD 文档的完整说明，请参见 [CIMD 认证](/zh/clients/auth/cimd) 页面。
