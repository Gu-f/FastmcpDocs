...
title: CIMD 认证
sidebarTitle: CIMD
description: 使用 Client ID Metadata Documents 提供可验证、基于域名的客户端身份。
icon: id-badge
tag: NEW
---
...
使用标准 OAuth 时，客户端会在连接到每个服务端时动态注册，每次都会收到一个新的 `client_id`。这种方式可以工作，但服务端无法验证你的客户端实际*是谁*，因为任何客户端都可以在注册时声明任意名称。
...
CIMD 是 MCP 服务端必须支持的服务端功能。FastMCP 的 OAuth 代理提供方（GitHub、Google、Auth0 等）默认支持 CIMD。服务端配置请参见 [OAuth 代理 CIMD 文档](/zh/servers/auth/oauth-proxy#安全)，其中包括 private key JWT 认证和安全细节。
