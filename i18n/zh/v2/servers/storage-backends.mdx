...
[OAuth 代理](/zh/v2/servers/auth/oauth-proxy) 和 OAuth 身份验证提供商使用存储来持久化 OAuth 客户端注册和上游令牌。**默认情况下，存储使用 `FernetEncryptionWrapper` 自动加密。** 提供自定义存储时，将其包装在 `FernetEncryptionWrapper` 中以加密静态的敏感 OAuth 令牌。
...
两个参数都是生产所必需的。**将您的存储包装在 `FernetEncryptionWrapper` 中以加密静态的敏感 OAuth 令牌** - 没有它，令牌将以明文存储。有关完整的设置详情，请参阅 [OAuth 令牌安全](/zh/v2/deployment/http#oauth-令牌安全性) 和 [密钥和存储管理](/zh/v2/servers/auth/oauth-proxy#密钥和存储管理)。

### 响应缓存中间件

[响应缓存中间件](/zh/v2/servers/middleware#日志记录中间件) 缓存工具调用、资源读取和提示请求。存储配置通过 `cache_storage` 参数传递：
...
[FastMCP 客户端](/zh/v2/clients/client) 使用存储在本地持久化 OAuth 令牌。默认情况下，令牌存储在内存中：
...
- [响应缓存中间件](/zh/v2/servers/middleware#日志记录中间件) - 使用存储进行缓存
- [OAuth 令牌安全](/zh/v2/deployment/http#oauth-令牌安全性) - 生产 OAuth 配置
- [HTTP 部署](/zh/v2/deployment/http) - 完整部署指南