...
<ResponseField name="ctx.elicit" type="异步方法">
  <Expandable title="参数">
    ...
    <ResponseField name="response_type" type="type" default="None">
      定义预期响应结构的 Python 类型（数据类、基本类型等）。请注意，征询响应受到 JSON Schema 类型的限制子集的限制。更多详细信息请参阅[支持的响应类型](#响应类型)。
    </ResponseField>
  </Expandable>
...
征询需要客户端实现征询处理程序。有关客户端如何处理这些请求的详细信息，请参阅[客户端征询](/zh/v2/clients/elicitation)。
