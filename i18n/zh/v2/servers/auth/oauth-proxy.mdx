...
OAuth 代理使 FastMCP 服务器能够与**不支持动态客户端注册（DCR）**的 OAuth 提供商进行认证。这包括几乎所有传统的 OAuth 提供商：GitHub、Google、Azure、AWS、Discord、Facebook 和大多数企业身份系统。对于支持 DCR 的提供商（如 Descope 和 WorkOS AuthKit），请使用 [`RemoteAuthProvider`](/zh/v2/servers/auth/remote-oauth)。
...
<Note>
  ...
  可以考虑使用 [`OIDC
  代理`](/zh/v2/servers/auth/oidc-proxy) 进行自动配置。OIDC 代理
...
<ParamField body="token_verifier" type="TokenVerifier" required>
  一个 [`TokenVerifier`](/zh/v2/servers/auth/token-verification) 实例，用于验证
  提供商的令牌
</ParamField>
...
<ParamField body="issuer_url" type="AnyHttpUrl | str | None">
  ...
  **何时使用：**
  ...
  有关完整的挂载示例，请参阅 [HTTP 部署指南](/zh/v2/deployment/http#挂载已认证服务器)。
</ParamField>
...
对于具有多个服务器或云部署的生产部署，请参阅 [存储后端](/zh/v2/servers/storage-backends) 了解可用选项。
...
<VersionBadge version="2.13.0" />
  ...
  **对于生产环境：**
  ...
  有关完整的生产设置，请参阅 [HTTP 部署 - OAuth 令牌安全](/zh/v2/deployment/http#oauth-令牌安全性)。
</ParamField>
...
请参阅 [令牌验证指南](/zh/v2/servers/auth/token-verification) 了解您提供商的详细设置说明。
...
代理创建的动态客户端将在其授权请求中自动包含这些作用域。请参阅下面的 [令牌验证](#令牌验证) 部分了解详细设置。
