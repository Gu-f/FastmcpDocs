...
在定义 FastMCP [工具](/zh/v2/servers/tools)、[资源](/zh/v2/servers/resources)、资源模板或[提示](/zh/v2/servers/prompts)时，您的函数可能需要与底层 MCP 会话交互或访问高级服务器功能。FastMCP 为此目的提供了 `Context` 对象。
...
有关完整文档和示例，请参阅[服务器日志记录](/zh/v2/servers/logging)。
### 客户端征询
...
有关详细示例和支持的响应类型，请参阅[用户征询](/zh/v2/servers/elicitation)。
...
有关全面的用法和高级技术，请参阅[LLM 采样](/zh/v2/servers/sampling)。
...
有关详细模式和示例，请参阅[进度报告](/zh/v2/servers/progress)。
...
**方法签名：**
- **`ctx.list_resources() -> list[MCPResource]`**：<VersionBadge version="2.13.0" /> 返回所有可用资源的列表
...
在请求中的中间件和工具调用之间存储和共享数据。上下文对象维护一个状态字典，特别适用于从[中间件](/zh/v2/servers/middleware)向工具传递信息。
