...
title: 后台任务
sidebarTitle: 后台任务
description: 以异步方式运行长耗时操作，并跟踪进度
icon: clock
tag: "NEW"
---
...
<Tip>
后台任务需要 `tasks` 可选 extra。请参见下方的[安装说明](#启用后台任务)。
</Tip>

FastMCP 实现了 MCP 后台任务协议（[SEP-1686](https://modelcontextprotocol.io/specification/2025-11-25/basic/utilities/tasks)），只需修改一个装饰器，就能为你的服务端提供生产就绪的分布式任务调度器。

<Tip>
**Docket 是什么？** FastMCP 的任务系统由 [Docket](https://github.com/chrisguidry/docket) 提供支持。Docket 最初由 [Prefect](https://prefect.io) 构建，用来支撑 [Prefect Cloud](https://www.prefect.io/prefect/cloud) 的托管任务调度和执行服务；该服务每天处理数百万个并发任务。Docket 现在已开源给社区。
</Tip>
...
MCP 后台任务不同之处在于它们是**协议原生**的。这意味着支持任务协议的 MCP 客户端可以通过标准 MCP 接口启动操作、接收进度更新并获取结果。协调发生在协议层，而不是你的应用代码内部。

## 启用后台任务

<VersionBadge version="3.0.0" /> 后台任务需要 `tasks` extra：
...
每个带有任务组件的 FastMCP 服务端都会自动启动一个**嵌入式 worker**。你不需要启动单独的 worker 进程，任务也能执行。
...
通过 `CurrentDocket()`，你可以调度额外后台任务、串联工作，并协调复杂工作流。完整 API 请参见 [Docket 文档](https://chrisguidry.github.io/docket/)，包括重试策略、超时和自定义依赖。
