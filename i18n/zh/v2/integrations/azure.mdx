...
本指南向您展示如何使用 **Azure OAuth**（Microsoft Entra ID）保护 FastMCP 服务器。由于 Azure 不支持动态客户端注册，此集成采用 [**OAuth 代理**](/zh/v2/servers/auth/oauth-proxy) 模式，将 Azure 的传统 OAuth 与 MCP 的身份验证需求衔接起来。FastMCP 会基于应用程序的 `client_id` 验证 Azure JWT。
