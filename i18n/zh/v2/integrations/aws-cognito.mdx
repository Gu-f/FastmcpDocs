...
本指南向您展示如何使用 **AWS Cognito 用户池**保护您的 FastMCP 服务器。由于 AWS Cognito 不支持动态客户端注册，此集成使用 [**OAuth 代理**](/zh/v2/servers/auth/oauth-proxy) 模式来桥接 AWS Cognito 的传统 OAuth 与 MCP 的身份验证需求。它还包括强大的 JWT 令牌验证，确保企业级身份验证。
...
更多参数细节请参阅 [OAuth 代理文档](/zh/v2/servers/auth/oauth-proxy#配置参数)。
</Note>
