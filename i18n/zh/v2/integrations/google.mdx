...
本指南将向您展示如何使用 **Google OAuth** 保护您的 FastMCP 服务器。由于 Google 不支持动态客户端注册，此集成使用 [**OAuth 代理**](/zh/v2/servers/auth/oauth-proxy) 模式来连接 Google 的传统 OAuth 与 MCP 的身份验证要求。
...
更多参数详情请参阅 [OAuth 代理文档](/zh/v2/servers/auth/oauth-proxy#配置参数)。
</Note>
...
<ParamField path="FASTMCP_SERVER_AUTH_GOOGLE_ISSUER_URL" default="使用 BASE_URL">
