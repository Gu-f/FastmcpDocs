...
在上面的示例中，我们使用 `stdio` 传输连接到了本地服务端。因为使用的是 FastMCP 客户端，所以只需更改客户端配置，你也可以使用 FastMCP 支持的任何[传输](/zh/v2/clients/transports)或[认证](/zh/v2/clients/auth)方法，连接到任意本地或远程 MCP 服务端。
