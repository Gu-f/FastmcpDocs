...
<Note>
**为了获得最佳体验，请使用 FastMCP 的一流集成：** [`fastmcp install claude-code`](/zh/v2/integrations/claude-code)、[`fastmcp install claude-desktop`](/zh/v2/integrations/claude-desktop) 或 [`fastmcp install cursor`](/zh/v2/integrations/cursor)。对于高级用例和不支持的客户端，请使用 MCP JSON 生成。
</Note>
...
### Claude Desktop
<Note>
**首选 [`fastmcp install claude-desktop`](/zh/v2/integrations/claude-desktop)** 进行自动安装。对于高级配置需求，请使用 MCP JSON。
</Note>
将 `mcpServers` 对象复制到 `~/.claude/claude_desktop_config.json`

### Cursor
<Note>
**首选 [`fastmcp install cursor`](/zh/v2/integrations/cursor)** 进行自动安装。对于高级配置需求，请使用 MCP JSON。
</Note>
添加到 `~/.cursor/mcp.json`
