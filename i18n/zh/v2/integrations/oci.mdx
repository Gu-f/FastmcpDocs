...
本指南展示如何使用 **OCI IAM OAuth** 保护你的 FastMCP 服务端。由于 OCI IAM 不支持动态客户端注册，此集成使用 [**OIDC 代理**](/zh/v2/servers/auth/oidc-proxy) 模式，将 OCI 的传统 OAuth 与 MCP 的认证要求连接起来。
...
这些参数的完整说明请参见 [OAuth 代理文档](/zh/v2/servers/auth/oauth-proxy#配置参数)。
</Note>
