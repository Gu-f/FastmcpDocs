...
本指南展示如何使用 WorkOS 的 **AuthKit** 保护 FastMCP 服务端。AuthKit 是完整的身份认证和用户管理解决方案。此集成使用带 [RFC 8707](https://www.rfc-editor.org/rfc/rfc8707.html) resource indicators 的 [**Remote OAuth**](/zh/v2/servers/auth/remote-oauth) 模式：AuthKit 会签发 `aud` claim 绑定到服务端资源 URL 的 token，FastMCP 会自动验证该 claim。
