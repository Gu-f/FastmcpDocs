...
title: FastMCP 客户端
sidebarTitle: 概览
description: 通过类型良好、Pythonic 的接口与 MCP 服务端交互的编程式客户端。
icon: user-robot
---
...
**内存中传输**会在同一个 Python 进程内直接连接到 FastMCP 服务端实例。适用于测试和开发，尤其是你想消除子进程和网络复杂性时。服务端会共享当前进程的环境和内存空间。
...
**STDIO 传输**会将服务端作为子进程启动，并通过 stdin/stdout 管道通信。这是 Claude Desktop 等桌面客户端使用的标准机制。子进程运行在隔离环境中，因此必须显式传入服务端所需的任何环境变量。
...
**HTTP 传输**连接到作为 Web 服务运行的服务端。适用于生产部署，此时服务端独立运行并管理自己的生命周期。
...
有关身份认证 headers、会话持久化和多服务端配置等详细配置选项，请参阅[传输](/zh/v2/clients/transports)。
...
**工具**是客户端可以携带参数执行的服务端函数。使用 `call_tool()` 调用它们，并接收结构化结果。
...
有关版本选择、错误处理和结构化输出等详细文档，请参阅[工具](/zh/v2/clients/tools)。

**资源**是客户端可以读取的数据源，可以是静态资源，也可以是模板化资源。使用 `read_resource()` 并通过 URI 访问它们。
...
有关模板和二进制内容等详细文档，请参阅[资源](/zh/v2/clients/resources)。

**提示词**是可复用的消息模板，可以接受参数。使用 `get_prompt()` 获取渲染后的提示词。
...
有关参数序列化等详细文档，请参阅[提示词](/zh/v2/clients/prompts)。
...
- **[采样](/zh/v2/clients/sampling)** - 响应服务端的 LLM 请求
- **[用户征询](/zh/v2/clients/elicitation)** - 处理服务端对用户输入的请求
- **[进度](/zh/v2/clients/progress)** - 监控长时间运行的操作
- **[日志](/zh/v2/clients/logging)** - 处理服务端日志消息
- **[Roots](/zh/v2/clients/roots)** - 向服务端提供本地上下文
