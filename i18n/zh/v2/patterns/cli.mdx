...
1. **[推断服务器实例](#推断服务器实例)**：`server.py` - 导入模块并查找名为 `mcp`、`server` 或 `app` 的 FastMCP 服务器实例。如果未找到此类对象则报错。
2. **[显式服务器入口点](#显式服务器入口点)**：`server.py:custom_name` - 导入并使用指定的服务器入口点
3. **[工厂函数](#工厂函数)**：`server.py:create_server` - 调用指定的函数（同步或异步）创建服务器实例
4. **[远程服务器代理](#远程服务器代理)**：`https://example.com/mcp-server` - 连接到远程服务器并创建**本地代理服务器**
...
有关 fastmcp.json 的详细文档，请参阅[服务器配置](/zh/v2/deployment/server-configuration)。
