---
title: 运行您的服务器
sidebarTitle: 运行您的服务器
description: 学习如何在本地运行您的 FastMCP 服务器进行开发和测试
icon: circle-play
---

FastMCP 服务器可以根据您的需求以不同方式运行。本指南专注于在本地运行服务器进行开发和测试。有关生产部署到 URL 的信息，请参阅[HTTP 部署](/zh/v2/deployment/http)指南。
...
有关具有身份验证和高级配置的生产 HTTP 部署，请参阅[HTTP 部署](/zh/v2/deployment/http)指南。
...
有关更多 CLI 功能，包括使用 MCP Inspector 的开发模式，请参阅 [CLI 文档](/zh/v2/patterns/cli)。
...
自定义路由由与 MCP 端点相同的 Web 服务器提供服务。它们在您域名的根目录下可用，而 MCP 端点位于 `/mcp/`。对于更复杂的 Web 应用程序，请考虑[将您的 MCP 服务器挂载到 FastAPI 或 Starlette 应用中](/zh/v2/deployment/http#与-web-框架集成)。
...
有关更多 ASGI 部署模式，请参阅[HTTP 部署](/zh/v2/deployment/http)指南。