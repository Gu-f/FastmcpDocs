...
title: 生成式 UI
sidebarTitle: 生成式 UI
description: 让 LLM 即时构建自定义 Prefab UI。
...
1. 宿主通过 `ontoolinputpartial` 将部分参数转发给应用
2. 渲染器提取不断增长的 `code` 字符串
3. 浏览器端 Pyodide 执行任何能够成功编译的内容
4. 用户看到组件随着代码编写逐步出现
...
- **[交互式工具](/zh/apps/prefab)**：LLM 将使用的组件构建块
- **[Prefab 组件参考](https://prefab.prefect.io/docs/components)**：完整组件库
- **[开发](/zh/apps/development)**：使用 `fastmcp dev apps` 在本地预览生成式工具
