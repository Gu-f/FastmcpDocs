...
description: 将交互式 UI 接入后端工具，并获得受管理的可见性和组合安全性。
icon: puzzle-piece
tag: NEW
---
...
<PrefabDemoFrame demo="contacts" height="650px" title="联系人应用演示" />
...
这是个合理的问题。任何[交互式工具](/zh/apps/prefab)都可以调用服务端工具；你完全可以把 `CallTool("add_note")` 放进普通的 `@mcp.tool(app=True)` 中。一两个工具时这样可行。但应用变大后，事情会变难：
...
- **[交互式工具](/zh/apps/prefab)**：构建块，包括图表、表格、仪表盘和响应式 state
- **[示例](/zh/apps/examples)**：完整的可运行服务端
- **[开发](/zh/apps/development)**：在本地预览和测试应用工具
- **[Prefab UI 文档](https://prefab.prefect.io)**：完整组件参考
