...
title: 快速开始
sidebarTitle: 快速开始
description: 在一分钟内构建你的第一个 FastMCP 应用。
icon: rocket
tag: NEW
---
...
<PrefabDemoFrame demo="team-directory" height="545px" title="团队目录演示" />
...
`apps` extra 会引入 [Prefab](https://prefab.prefect.io)，这是用于构建应用 UI 的 Python 组件库。
...
<Frame>
  <img src="/apps/images/app-quickstart-dev-2.png" alt="在 fastmcp dev apps 预览中渲染的团队目录，显示了饼图、可搜索表格，以及点击一行后出现的详情卡片" />
</Frame>
...
<PrefabDemoFrame demo="team-directory-reactive" height="675px" title="响应式团队目录演示" />
...
- **[交互式工具](/zh/apps/prefab)**：图表、表格、仪表盘、响应式状态，以及实时演示
- **[FastMCPApp](/zh/apps/fastmcp-app)**：当 UI 需要回调你的服务端时使用（表单、搜索、CRUD）
