...
title: 文件上传
sidebarTitle: 文件上传
description: 为任意 MCP 服务端提供拖放文件上传
icon: upload
tag: NEW
---
...
<Frame>
  <img src="/apps/images/app-file-upload.png" alt="Goose 中显示的 FileUpload provider，包含用于上传文件的拖放区域" />
</Frame>
...
<Warning>
在 **无状态 HTTP** 模式下，每个请求都会创建带有新 ID 的新会话对象。一次请求中存储的文件（例如 UI 上传）对下一次请求（例如 LLM 调用 `list_files`）不可见。你**必须**覆写 `_get_scope_key`，使用稳定标识符，例如来自认证 token 的用户 ID。
</Warning>
