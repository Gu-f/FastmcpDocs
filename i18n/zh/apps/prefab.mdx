...
title: 交互式工具
sidebarTitle: 交互式工具
description: 将你的工具变成交互式 UI，包含图表、表格和仪表盘。
icon: palette
tag: NEW
---
...
<PrefabDemoFrame demo="dashboard" height="680px" title="销售仪表盘演示" />

信不信由你，那个仪表盘就是一个 FastMCP 工具。图表有工具提示，表格可以排序，徽章会根据交易阶段设置样式。整个东西大约 40 行 Python，用户会在对话中直接看到它，而不是面对一墙 JSON。

本页每个示例背后的模式都一样：给工具添加 `app=True`，用 [Prefab](https://prefab.prefect.io) 组件构建 UI，并将其作为 `PrefabApp` 返回。Prefab 有 [100+ 个组件](https://prefab.prefect.io/docs/components)，涵盖数据表、图表、表单和进度条等。你用 Python 组合它们；宿主将它们渲染为实时交互式应用。

## 从表格开始

大多数工具都会返回用户想要探索的数据。`DataTable` 通常是最小但有用的升级：你的数据会从 JSON 数据块变成可搜索、可排序的表格：

<PrefabDemoFrame demo="data-table" height="530px" title="数据表演示" />
...
<PrefabDemoFrame demo="bar-chart" height="430px" title="柱状图演示" />
...
<PrefabDemoFrame demo="pie-chart" height="410px" title="饼图演示" />
...
关于堆叠、曲线、自定义颜色等内容，请参阅 [Prefab 图表文档](https://prefab.prefect.io/docs/components)。
...
<PrefabDemoFrame demo="reactive" height="500px" title="响应式销售演示" />
...
`Rx` 是响应式引用：`Rx("region")` 会编译为渲染器实时求值的表达式。它支持算术、比较、格式化管道（`.currency()`、`.percent()`）和三元条件（`.then()`）。完整 state 系统请参阅 [Prefab state 文档](https://prefab.prefect.io/docs/concepts/state)和[表达式文档](https://prefab.prefect.io/docs/concepts/expressions)。
...
- **[FastMCPApp](/zh/apps/fastmcp-app)**：当 UI 需要调用后端工具时使用（表单、搜索、CRUD）
- **[生成式 UI](/zh/apps/generative)**：让 LLM 在运行时设计 UI
- **[自定义 HTML](/zh/apps/low-level)**：当 Prefab 不够用时使用（地图、3D、你自己的框架）
- **[示例](/zh/apps/examples)**：可以今天就运行的完整服务端
- **[开发](/zh/apps/development)**：使用 `fastmcp dev apps` 在本地预览你的工具
- **[Prefab UI](https://prefab.prefect.io)**：完整组件参考，包含 100+ 个组件、主题和高级模式
