...
title: 架构
sidebarTitle: 架构
description: 了解 FastMCP 应用在底层如何工作：从 Python 到像素。
icon: sitemap
---
...
构建应用并不需要先读完这一页。它适合在 UI 没有按预期渲染、UI 工具调用没有抵达服务端，或者你正在编写[自定义 HTML 应用](/zh/apps/low-level)并需要直接理解协议时参考。
...
渲染器运行在沙盒 iframe 中，并使用 `postMessage` 与宿主通信。该协议遵循 [MCP Apps extension](https://modelcontextprotocol.io/docs/extensions/apps) 规范：
...
`@modelcontextprotocol/ext-apps` JavaScript SDK 提供了 `App` 类（有时称为 AppBridge）来管理 `postMessage` 握手。它处理连接协商、工具结果交付、服务端工具调用以及宿主上下文（安全区域边距、主题偏好）。Prefab 渲染器在内部使用它；只有在构建[自定义 HTML 应用](/zh/apps/low-level)时，你才需要直接接触它。
