...
虽然内存测试涵盖了大部分单元测试需求，但您偶尔需要测试实际的网络传输，例如 HTTP 或 SSE。FastMCP 提供两种方式：使用 AnyIO 任务组的进程内异步服务器（推荐），以及单独的子进程服务器（特殊场景）。

#### 进程内网络测试（推荐）

<VersionBadge version="2.13.0" />

对于大多数网络传输测试，请结合 AnyIO 任务组使用 `run_server_async`。这会在同一进程中作为任务运行服务器，具备快速、可预测且完全支持调试器的特性：
...
`conftest.py` 全局提供的 `task_group` fixture 会自动处理服务器的生命周期与清理。与基于子进程的测试相比，这种方法更快且错误提示更清晰。

#### 子进程测试（特殊场景）

对于需要完全进程隔离的测试（例如 STDIO 传输或验证子进程行为），请使用 `run_server_in_process`：
...
`run_server_in_process` 实用工具会自动处理服务器生命周期、端口分配和清理。仅在确实需要子进程隔离时使用它，因为它比进程内测试更慢也更难调试。FastMCP 会使用 `client_process` 标记在 CI 中隔离此类测试。
