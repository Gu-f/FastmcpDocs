...
本文档跟踪 FastMCP v3.0 的主要特性，用于准备发布说明。

## 3.0.0rc1

### SamplingTool 转换辅助工具

服务端工具（FunctionTool 和 TransformedTool）现在可以通过 `SamplingTool.from_callable_tool()` 直接传给采样方法 ([#3062](https://github.com/PrefectHQ/fastmcp/pull/3062)). 以前，使用 `@mcp.tool` 定义的工具必须重新创建为函数，才能在 `ctx.sample()` 中使用。 现在 `ctx.sample()` 和 `ctx.sample_step()` 可以直接接受这些工具实例。
...
result = await ctx.sample(
    ...
    tools=[search]  # FunctionTool 可以直接工作！
)
```

### Google GenAI 采样处理器

FastMCP 现在包含面向 Google Gemini 模型的采样处理器 ([#2977](https://github.com/jlowin/fastmcp/pull/2977)). 这让 MCP 客户端可以通过采样协议使用 Google 的 GenAI 模型，包括完整的工具调用支持。
...
handler = GoogleGenaiSamplingHandler(
    ...
    client=GoogleGenaiClient(),  # 可选 - 未提供时会自动创建
)

# 与 MCP 采样配合使用（处理器在构造 Client 时配置）
...
主要特性：
- 将 MCP 工具 schema 转换为 Google 的函数调用格式
- 支持所有实现函数调用的 Google GenAI 模型
- 处理工具 schema 中的可空类型、嵌套对象和数组
- 将工具选择（`auto`、`required`、`none`）正确映射到 Google 配置
- 保留 MCP 采样参数中的模型偏好

该处理器加入了已有的 Anthropic 和 OpenAI 处理器，为跨提供方的模型无关采样提供一致接口。

### 采样中的并发工具执行

当 LLM 在单个采样响应中返回多个工具调用时，现在可以并发执行这些调用 ([#3022](https://github.com/PrefectHQ/fastmcp/pull/3022)). 默认行为仍然是顺序执行；可通过 `tool_concurrency` 选择启用。 即使启用了并发，工具也可以声明 `sequential=True` 来强制顺序执行。

```python
result = await context.sample(
    messages="获取纽约和洛杉矶的天气",
    tools=[fetch_weather],
    tool_concurrency=0,  # 不限制并行执行数量
)
```

### OpenAPI `validate_output` 选项

`OpenAPIProvider` 和 `FastMCP.from_openapi()` 现在接受 `validate_output=False` 以跳过输出 schema 校验 ([#3134](https://github.com/PrefectHQ/fastmcp/pull/3134)). 当后端并不符合自身 OpenAPI 响应 schema 时很有用；结构化 JSON 仍会正常传递，只是禁用了严格的 schema 检查。
...
### 认证令牌注入与 Azure OBO 依赖

新增依赖注入能力，可在工具参数中直接访问已认证用户的令牌 ([#2918](https://github.com/PrefectHQ/fastmcp/pull/2918)). 适用于任何认证提供方。
...
对于 Azure/Entra，新的 `fastmcp[azure]` extra 增加了 `EntraOBOToken`，可声明式处理 On-Behalf-Of 令牌交换：
...
async def get_emails(
    ...
    # graph_token 已就绪，OBO 交换已自动完成
    ...
```

### `generate-cli` Agent Skill 生成

`fastmcp generate-cli` 现在会在生成 CLI 脚本的同时生成 `SKILL.md` ([#3115](https://github.com/PrefectHQ/fastmcp/pull/3115)) — 这是一个 Claude Code agent skill，其中包含每个工具预先计算好的调用语法. 读取该 skill 的 agent 无需运行 `--help` 即可立即调用工具。 默认启用；传入 `--no-skill` 可选择关闭。

### 后台任务通知队列

后台任务现在使用分布式 Redis 通知队列来可靠投递 ([#2906](https://github.com/PrefectHQ/fastmcp/pull/2906)). 用户征询从轮询切换为 BLPOP（单次阻塞调用，而不是每小时约 7,200 次往返），通知投递最多重试 3 次，并基于 TTL 过期。

### 异步认证检查

认证检查函数现在可以是 `async`，从而支持依赖异步操作的授权决策，例如通过 `Context.get_state` 读取服务端状态或调用外部服务 ([#3150](https://github.com/PrefectHQ/fastmcp/issues/3150)). 同步和异步检查可以自由混用。 以前，将异步函数作为认证检查传入会静默通过（因为协程对象为真值）。

### schema 中可选的 `$ref` 解引用

Schema `$ref` 解引用会内联所有 `$defs`，以兼容不处理 `$ref` 的 MCP 客户端；现在它由 `dereference_schemas` 构造函数关键字参数控制 ([#3141](https://github.com/PrefectHQ/fastmcp/issues/3141)). 默认值为 `True`（启用解引用），因为不合规客户端较为常见，而且失败模式是服务端作者难以诊断的静默损坏。当你确定客户端能处理 `$ref` 并希望 schema 更小时，可以选择关闭：

```python
mcp = FastMCP("my-server", dereference_schemas=False)
```

解引用以中间件（`DereferenceRefsMiddleware`）形式实现，并在提供服务时运行，因此 schema 会以保留 `$ref` 的形式存储，只在发送给客户端时内联。

### 破坏性变更：移除已弃用的 `FastMCP()` 构造函数关键字参数

`FastMCP.__init__` 中已经移除了 16 个已弃用的关键字参数。现在传入其中任何一个都会抛出带迁移提示的 `TypeError`。环境变量（例如 `FASTMCP_HOST`）仍然可用；移动的只是构造函数关键字参数。

**传输/服务端设置** (`host`, `port`, `log_level`, `debug`, `sse_path`, `message_path`, `streamable_http_path`, `json_response`, `stateless_http`): 请按需传给 `run()`、`run_http_async()` 或 `http_app()`，或者通过环境变量设置。
...
**重复项处理** (`on_duplicate_tools`, `on_duplicate_resources`, `on_duplicate_prompts`): 请使用统一的 `on_duplicate=` 参数。

**标签过滤** (`include_tags`, `exclude_tags`): 请在构造后使用 `server.enable(tags=..., only=True)` 和 `server.disable(tags=...)`。

**工具序列化器** (`tool_serializer`): 请改为从工具返回 `ToolResult`。

**工具转换** (`tool_transformations`): 请在构造后使用 `server.add_transform(ToolTransform(...))`。

`_deprecated_settings` 属性和 `.settings` 属性也已移除。`ExperimentalSettings` 已被删除（死代码）。

### 破坏性变更：`ui=` 重命名为 `app=`

MCP Apps 装饰器参数已从 `ui=ToolUI(...)` / `ui=ResourceUI(...)` 重命名为 `app=AppConfig(...)` ([#3117](https://github.com/PrefectHQ/fastmcp/pull/3117)). `ToolUI` 和 `ResourceUI` 已合并为单个 `AppConfig` 类。线缆格式保持不变。完整详情请参见 beta2 下的 MCP Apps 小节。
## 3.0.0beta2

### CLI: `fastmcp list` and `fastmcp call`

新增客户端侧 CLI 命令，可查询并调用任何 MCP 服务端上的工具，包括远程 URL、本地 Python 文件、MCPConfig JSON 或任意 stdio 命令。对于让不内置 MCP 支持的 LLM 通过 shell 命令访问 MCP 工具尤其有用。
...
主要特性：
- 工具参数会使用工具的 JSON schema 自动强制转换 (`limit=5` → int)
- 单个 JSON 对象可以作为位置参数使用，也可与 `key=value` 和 `--input-json` 搭配
- `--input-schema` / `--output-schema` 用于完整 JSON schema，`--json` 用于机器可读输出
- `--transport sse` 用于 SSE 服务端，`--command` 用于 stdio 服务端
- HTTP 目标自动 OAuth（如果服务端不要求认证则无操作）
- 工具名称模糊匹配会在拼写错误时建议替代项
- 对执行过程中请求用户输入的工具提供交互式终端用户征询

文档： [CLI Querying](/zh/cli/client)
...
`--reload` 标志现在会监视一组完整的文件类型，使其适用于带前端包的 MCP apps ([#3028](https://github.com/PrefectHQ/fastmcp/pull/3028)). 它以前仅限 `.py` 文件，现在会监视 JavaScript、TypeScript、HTML、CSS、配置文件和媒体资源。

### CLI：fastmcp install stdio

新的 `fastmcp install stdio` 命令会生成完整的 `uv run` 命令，用于通过 stdio 运行 FastMCP 服务端 ([#3032](https://github.com/PrefectHQ/fastmcp/pull/3032)).
...
该命令会自动检测项目目录，并生成合适的 `uv run` 调用，使 FastMCP 服务端更容易与 MCP 客户端集成。

### CIMD（客户端 ID 元数据文档）

CIMD 为经过 OAuth 认证的 MCP 服务端提供了动态客户端注册的替代方案。 客户端无需动态注册到每个服务端，而是在 HTTPS URL 上托管一个静态 JSON 文档。该 URL 会成为客户端的 `client_id`，服务端则通过域名所有权验证身份。
...
关键细节：
- CIMD URL 必须使用 HTTPS，并且路径不能是根路径
- `token_endpoint_auth_method` limited to `none` or `private_key_jwt` (不使用共享密钥)
- `redirect_uris` in CIMD documents support wildcard port patterns (`http://localhost:*/callback`)
- 服务端使用标准 HTTP 缓存获取并缓存文档 (ETag, Last-Modified, Cache-Control)
- CIMD 是协议级特性 — 任何实现该规范的认证提供方都可以支持它

文档： [CIMD Authentication](/zh/clients/auth/cimd), [OAuth Proxy CIMD config](/zh/servers/auth/oauth-proxy#安全)

### 预注册 OAuth 客户端

对于客户端已注册的服务端，`OAuth` 客户端辅助工具现在接受 `client_id` 和 `client_secret` 参数 ([#3086](https://github.com/PrefectHQ/fastmcp/pull/3086)). 这会完全绕过动态客户端注册 — useful when DCR is disabled, or when the server has pre-provisioned credentials for your application.
...
The static credentials are injected before the OAuth flow begins, so the client never attempts DCR. If the server rejects the credentials, the error surfaces immediately rather than retrying with fresh registration (which can't help for fixed credentials). 公共客户端可以省略 `client_secret`。

文档： [Pre-Registered Clients](/zh/clients/auth/oauth#预注册客户端)
...
文档： [Generate CLI](/zh/cli/generate-cli)
...
Also adds a full integration guide at [Goose Integration](/zh/integrations/goose).

### ResponseLimitingMiddleware

新增用于控制工具响应大小的中间件，防止大型输出淹没 LLM 上下文窗口 ([#3072](https://github.com/PrefectHQ/fastmcp/pull/3072)). Text responses are truncated at UTF-8 character boundaries; structured responses (tools with `output_schema`) raise `ToolError` since truncation would corrupt the schema.
...
- 可配置大小限制 (default 1MB)
- 通过 `tools` 参数按工具过滤
- 将大小元数据添加到结果的 `meta` 字段以便监控
- 可配置 `raise_on_structured` 和 `raise_on_unstructured` 行为

文档： [Middleware](/zh/servers/middleware)
...
**未来阶段** will add a component DSL for building UIs declaratively, an in-repo renderer, and a `FastMCPApp` class.

实现： `fastmcp_slim/fastmcp/server/apps.py` (models and constants), with integration points in `server.py` (decorator parameters), `low_level.py` (extension advertisement), and `context.py` (`client_supports_extension` method).
...
文档： `docs/servers/transforms/transforms.mdx`, `docs/servers/visibility.mdx`
...
文档： `docs/servers/transforms/resources-as-tools.mdx`, `docs/servers/transforms/prompts-as-tools.mdx`
...
文档： `docs/servers/visibility.mdx`
...
文档： [FileSystemProvider](/zh/servers/providers/filesystem)
...
文档： [Skills Provider](/zh/servers/providers/skills)
...
文档： [Telemetry](/zh/servers/telemetry)
...
文档： [Pagination](/zh/servers/pagination)
...
文档： [Lifespan](/zh/servers/lifespan)
