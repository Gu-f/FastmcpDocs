...
完整的 `fastmcp.json` 格式请参阅[服务端配置](/zh/deployment/server-configuration)。
...
<Warning>
`fastmcp run` 会完全忽略 `if __name__ == "__main__"` 代码块。该代码块中的任何设置代码都不会执行。如果需要运行初始化逻辑，请使用[工厂函数](/zh/cli/overview)。
</Warning>
...
`fastmcp dev apps` 会为带有 [Prefab App 工具](/zh/apps/prefab)的服务端启动基于浏览器的预览 UI。它会在一个端口启动 MCP 服务端，并在另一个端口启动本地开发 UI，从而提供一个实时交互式选择器，让你无需完整 MCP 宿主客户端就能调用应用工具并查看渲染输出。
...
`fastmcp dev inspector` 会在 [MCP Inspector](https://github.com/modelcontextprotocol/inspector) 中启动你的服务端。MCP Inspector 是一个基于浏览器的工具，用于交互式测试 MCP 服务端。默认启用自动重载，因此保存更改时服务端会重启。
