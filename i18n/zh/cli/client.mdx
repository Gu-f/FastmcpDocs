...
title: 客户端命令
sidebarTitle: 客户端
description: 列出工具、调用工具并发现已配置的服务端
icon: satellite-dish
---
...
对于期望本地 stdio 命令但需要连接远程 HTTP 服务端的 MCP 宿主，请使用 [`fastmcp-remote`](/zh/clients/fastmcp-remote)。它为宿主配置提供一个小型独立桥接器，而 `fastmcp list` 和 `fastmcp call` 则继续专注于从终端直接检查和调用。
