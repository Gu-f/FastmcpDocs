...
title: 生成 CLI
sidebarTitle: 生成 CLI
description: 从任意 MCP 服务端搭建独立的类型化 CLI
...
MCP 工具 schema 已经包含 CLI 框架所需的一切：参数名称、类型、描述、必填/可选状态和默认值。`generate-cli` 会将这些信息映射到 [cyclopts](https://cyclopts.readthedocs.io/) 命令，因此 JSON Schema 类型会变成 Python 类型注解，描述会变成 `--help` 文本，必填参数会变成强制标志。

## 生成脚本

将命令指向任意[服务端目标](/zh/cli/overview#服务端目标)，它就会写出 CLI 脚本：
...
除了 CLI 脚本，`generate-cli` 还会写出一个 `SKILL.md` 文件，这是一个 [Claude Code agent skill](https://docs.anthropic.com/en/docs/agents-and-tools/claude-code/skills)，记录每个工具的精确调用语法、参数标志、类型和描述。Agent 可以立即使用该 CLI，无需运行 `--help` 或试探标志名称。
...
生成的脚本是*客户端*，不是服务端：它会在每次调用时连接服务端，而不是把服务端打包进去。顶部的 `CLIENT_SPEC` 变量保存解析后的传输方式（URL 字符串，或内置命令和参数的 `StdioTransport`）。
