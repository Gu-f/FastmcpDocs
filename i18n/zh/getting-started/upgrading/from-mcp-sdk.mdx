---
title: 从 MCP SDK 升级
sidebarTitle: "从 MCP SDK"
description: 从 MCP Python SDK 中的 FastMCP 升级到独立的 FastMCP 框架
icon: up
---
...
<Prompt description="将此提示词连同你的服务端代码复制到任意 LLM 中，以获得自动升级指导。">
你正在将一个 MCP 服务端从 FastMCP 1.0（`mcp` 包 v1 随附）升级到独立 FastMCP 3.0。请分析给出的代码，找出所有需要修改的地方。完整升级指南位于 https://gofastmcp.com/getting-started/upgrading/from-mcp-sdk，完整 FastMCP 文档位于 https://gofastmcp.com，请获取这些内容作为完整上下文。

步骤 1 — 导入（所有服务端都需要）：
将 "from mcp.server.fastmcp import FastMCP" 改为 "from fastmcp import FastMCP"。

步骤 2 — 构造函数关键字参数（仅当 FastMCP() 接收传输设置时）：
FastMCP() 不再接受：host、port、log_level、debug、sse_path、streamable_http_path、json_response、stateless_http。
修复方式：改为传给 run()。
之前：`mcp = FastMCP("server", host="0.0.0.0", port=8080); mcp.run()`
之后：`mcp = FastMCP("server"); mcp.run(transport="http", host="0.0.0.0", port=8080)`

步骤 3 — 提示词（仅当直接使用 PromptMessage 或返回 dict 时）：
mcp.types.PromptMessage 已由 fastmcp.prompts.Message 替代。
之前：`PromptMessage(role="user", content=TextContent(type="text", text="Hello"))`
之后：`Message("Hello")`，role 默认为 "user"，并接受普通字符串。
另外：如果提示词返回 `{"role": "user", "content": "..."}` 这样的原始 dict，必须改为 Message 对象或普通字符串。MCP SDK 的 FastMCP 1.0 会静默地把 dict 强制转换成消息；独立 FastMCP 要求返回有类型的对象。

步骤 4 — 其他 MCP 导入（仅当直接从 mcp.* 导入时）：
直接从 `mcp` 包导入（例如 `import mcp.types`、`from mcp.server.stdio import stdio_server`）仍然可用，因为 FastMCP 将 `mcp` 作为依赖包含在内。不过，如果 FastMCP 提供了等价 API，优先使用 FastMCP 自己的 API：
- 工具返回中的 mcp.types.TextContent → 直接返回普通 Python 值（str、int、dict 等）
- mcp.types.ImageContent → fastmcp.utilities.types.Image
- from mcp.server.stdio import stdio_server → 不需要，mcp.run() 会处理传输

步骤 5 — 装饰器（仅当把被装饰函数当对象使用时）：
@mcp.tool、@mcp.resource、@mcp.prompt 现在返回原始函数，而不是组件对象。访问被装饰结果上的 .name 或 .description 的代码需要更新。可以临时设置 FASTMCP_DECORATOR_MODE=object 来恢复 v1 行为（这个兼容设置本身也已弃用）。

对发现的每个问题，请展示原始行，解释发生了什么变化，并给出修正后的代码。
</Prompt>
