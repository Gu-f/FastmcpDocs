...
sidebarTitle: "从 MCP Low-Level SDK"
description: 将你的 MCP 服务端从低层 Python SDK 的 Server 类升级到 FastMCP
icon: up
---
...
<Note>
Already using FastMCP 1.0 via `from mcp.server.fastmcp import FastMCP`? Your upgrade is simpler — see the [FastMCP 1.0 upgrade guide](/zh/getting-started/upgrading/from-mcp-sdk) instead.
</Note>
...
The `Context` object provides logging (`ctx.debug()`, `ctx.info()`, `ctx.warning()`, `ctx.error()`), progress reporting (`ctx.report_progress()`), resource subscriptions, session state, and more. See [Context](/zh/servers/context) for the full API.
...
- **[Server composition](/zh/servers/composition)** — Mount sub-servers to build modular applications
- **[Middleware](/zh/servers/middleware)** — Add logging, rate limiting, error handling, and caching
- **[Proxy servers](/zh/servers/providers/proxy)** — Create a proxy to any existing MCP server
- **[OpenAPI integration](/zh/integrations/openapi)** — Generate an MCP server from an OpenAPI spec
- **[Authentication](/zh/servers/auth/authentication)** — Built-in OAuth and token verification
- **[Testing](/zh/servers/testing)** — Test your server directly in Python without running a subprocess
