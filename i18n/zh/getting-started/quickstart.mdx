...
如果你还没有安装 FastMCP，请先按照[安装说明](/zh/getting-started/installation)操作。
...
工具通常返回文本，但任何工具也可以返回交互式 UI。给工具装饰器添加 `app=True`，并返回一个 [Prefab](https://prefab.prefect.io) 组件，宿主就会在对话中将它渲染为图表、表格、表单或其他可视元素。这需要安装 `apps` extra（`pip install "fastmcp[apps]"`）。
...
你可以使用 `fastmcp dev apps my_server.py` 在本地预览 app 工具，无需 MCP 宿主。完整指南请参阅 [Apps 概览](/zh/apps/overview)，其中包括状态管理、表单、图表，以及与服务端连接的交互能力。
...
更多细节请参阅 [Prefect Horizon 指南](/zh/deployment/prefect-horizon)。
