---
title: 安装
description: 安装 FastMCP 并验证你的环境
...
有关任务系统的详细信息，请参阅[后台任务](/zh/servers/tasks)。
...
或者等待稳定版 v5 发布。详情请参阅[这个 issue](https://github.com/BrianPugh/cyclopts/issues/672)。
</Info>
## 升级

### 从 FastMCP 2.0 升级

有关破坏性变更和迁移步骤的完整列表，请参阅[升级指南](/zh/getting-started/upgrading/from-fastmcp-2)。

### 从 MCP SDK 升级

#### 从 FastMCP 1.0 升级

如果你通过 `mcp` 包使用 FastMCP 1.0（也就是以 `from mcp.server.fastmcp import FastMCP` 的方式导入 FastMCP），升级会很直接；对大多数服务端来说，只需要改一行导入。详情请参阅[完整升级指南](/zh/getting-started/upgrading/from-mcp-sdk)。

#### 从底层 Server API 升级

如果你直接基于 `mcp` 包的 `Server` 类构建服务端，使用 `list_tools()`/`call_tool()` 处理器并手写 JSON Schema，请参阅[迁移指南](/zh/getting-started/upgrading/from-low-level-sdk)获取完整演练。
...
有关公共 API、弃用实践和破坏性变更原则的详细信息，请参阅完整的[版本与发布策略](/zh/development/releases#版本策略)。

## 参与 FastMCP 贡献

想为 FastMCP 做贡献？请参阅[贡献指南](/zh/development/contributing)，了解以下内容：
- 设置开发环境
