...
发送到客户端的消息也会以 `DEBUG` 级别记录到 `fastmcp.server.context.to_client` 记录器中。
...
**返回值：**
- StreamableHTTP 传输的会话 ID，或其他传输的生成 ID。
