...
本指南展示如何使用 **Discord OAuth** 保护你的 FastMCP 服务端。由于 Discord 不支持 Dynamic Client Registration，此集成使用 [**OAuth Proxy**](/zh/servers/auth/oauth-proxy) 模式，将 Discord 的传统 OAuth 与 MCP 的身份认证要求连接起来。
...
有关这些参数的完整详情，请参阅 [OAuth Proxy 文档](/zh/servers/auth/oauth-proxy#配置参数)。
</Note>
