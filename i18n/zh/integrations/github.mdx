...
本指南将向您展示如何使用 **GitHub OAuth** 保护您的 FastMCP 服务器。由于 GitHub 不支持动态客户端注册，此集成使用 [**OAuth 代理**](/servers/auth/oauth-proxy) 模式来连接 GitHub 的传统 OAuth 与 MCP 的身份验证要求。
...
<Info>
要使用注册的 GitHub 提供程序，您必须设置 `FASTMCP_SERVER_AUTH=GITHUB`。了解更多关于 [注册提供程序](/servers/auth/authentication#registered-providers) 的信息。
</Info>

### 提供程序选择

<ParamField path="FASTMCP_SERVER_AUTH" default="Not set" required>
设置为 `GITHUB` 以使用具有默认配置的注册 GitHubProvider。
</ParamField>
...
需要的 GitHub 范围的逗号分隔列表（例如，`user,repo`）
</ParamField>
