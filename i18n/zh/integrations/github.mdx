...
更多参数详情请参阅 [OAuth 代理文档](/zh/servers/auth/oauth-proxy#配置参数)。
</Note>
...
用于 OAuth 元数据的 Issuer URL（默认等于 `BASE_URL`）。当挂载在路径前缀下时，请设置为根级 URL 以避免 404 日志。详见 [HTTP 部署指南](/zh/deployment/http#挂载已认证服务器)。
</ParamField>
