...
<Step title="获取 AWS Cognito 域名">
    在侧边导航中转到 **"品牌" → "域名"** 来查找或配置您的 AWS Cognito 域名：

    **选项 1：使用自动生成的域名**
    - 如果 AWS 已经自动创建了域名，请记住 **域名前缀**（`.auth.region.amazoncognito.com` 之前的部分）
    - 这个前缀就是您在 FastMCP 配置中将使用的内容

    **选项 2：创建自定义域名前缀**
    - 如果不存在域名或您想要更好的名称，请使用 **"操作"** 菜单删除现有域名并创建新域名
    - 在 **"创建 Cognito 域名"** 对话框中的 **"域名" → "Cognito 域名"** 下，输入在您所在 AWS 区域中可用的有意义的前缀（例如，`my-app`）
    - 只需记住您输入的 **域名前缀**（例如，`my-fastmcp-app`） - 这就是您在 FastMCP 配置中将使用的内容

    <Info>
    FastMCP AWS Cognito 提供者从您的前缀和区域自动构建完整域名，简化配置。
    </Info>
</Step>
