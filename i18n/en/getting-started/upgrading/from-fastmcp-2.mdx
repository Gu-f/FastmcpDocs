...
- `tool_serializer` — return [`ToolResult`](/en/servers/tools#custom-serialization) from your tools instead
...
If you were passing a `DiskStore` explicitly, you can either [switch to `FileTreeStore`](/en/servers/storage-backends) (recommended) or keep using `DiskStore` by adding the dependency yourself.

<Warning>
When switching to `FileTreeStore`, you **must** configure key and collection sanitization strategies. Without them, keys containing special characters (such as URL-based OAuth client IDs) will cause filesystem errors. See the [File Storage](/en/servers/storage-backends#file-storage) section for the recommended setup.
</Warning>
...
Calling `.enable()` or `.disable()` on a component object now raises `NotImplementedError`. See [Visibility](/en/servers/visibility) for the full API, including tag-based filtering and per-session visibility.
...
Without the extra, configuring a tool with `task=True` or `TaskConfig` will raise an import error at runtime. See [Background Tasks](/en/servers/tasks) for details.
...
See [OAuth Token Security](/en/deployment/http#oauth-token-security) for details.
