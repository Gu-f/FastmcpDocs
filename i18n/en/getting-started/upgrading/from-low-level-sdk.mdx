...
<Note>
Already using FastMCP 1.0 via `from mcp.server.fastmcp import FastMCP`? Your upgrade is simpler — see the [FastMCP 1.0 upgrade guide](/en/getting-started/upgrading/from-mcp-sdk) instead.
</Note>
...
The `Context` object provides logging (`ctx.debug()`, `ctx.info()`, `ctx.warning()`, `ctx.error()`), progress reporting (`ctx.report_progress()`), resource subscriptions, session state, and more. See [Context](/en/servers/context) for the full API.
...
- **[Server composition](/en/servers/composition)** — Mount sub-servers to build modular applications
- **[Middleware](/en/servers/middleware)** — Add logging, rate limiting, error handling, and caching
- **[Proxy servers](/en/servers/providers/proxy)** — Create a proxy to any existing MCP server
- **[OpenAPI integration](/en/integrations/openapi)** — Generate an MCP server from an OpenAPI spec
- **[Authentication](/en/servers/auth/authentication)** — Built-in OAuth and token verification
- **[Testing](/en/servers/testing)** — Test your server directly in Python without running a subprocess
