...
The `inspect` command supports the same local entrypoints as [`fastmcp run`](/en/cli/running): inferred instances, explicit entrypoints, factory functions, and `fastmcp.json` configs.
