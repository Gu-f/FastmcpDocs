...
| ------- | ------------ |
| [`run`](/en/cli/running) | Run a server (local file, factory function, remote URL, or config file) |
| [`dev apps`](/en/cli/running#previewing-apps) | Launch a browser-based preview UI for Prefab App tools |
| [`dev inspector`](/en/cli/running#development-with-the-inspector) | Launch a server inside the MCP Inspector for interactive testing |
| [`install`](/en/cli/install-mcp) | Install a server into Claude Code, Claude Desktop, Cursor, Gemini CLI, or Goose |
| [`inspect`](/en/cli/inspecting) | Print a server's tools, resources, and prompts as a summary or JSON report |
| [`list`](/en/cli/client) | List a server's tools (and optionally resources and prompts) |
| [`call`](/en/cli/client#calling-tools) | Call a single tool with arguments |
| [`discover`](/en/cli/client#discovering-configured-servers) | Find MCP servers configured in your editors and tools |
| [`generate-cli`](/en/cli/generate-cli) | Scaffold a standalone typed CLI from a server's tool schemas |
| [`project prepare`](/en/cli/running#pre-building-environments) | Pre-install dependencies into a reusable uv project |
| [`auth cimd`](/en/cli/auth) | Create and validate CIMD documents for OAuth |
...
Run [`fastmcp discover`](/en/cli/client#discovering-configured-servers) to see what's available on your machine.
