...
See [Server Configuration](/en/deployment/server-configuration) for the full `fastmcp.json` format.
...
<Warning>
`fastmcp run` completely ignores the `if __name__ == "__main__"` block. Any setup code in that block won't execute. If you need initialization logic to run, use a [factory function](/en/cli/overview#factory-functions).
</Warning>
