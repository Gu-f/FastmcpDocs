...
- **[Namespace](/en/servers/transforms/namespace)** - Prefix component names to prevent conflicts when composing servers
- **[Tool Transformation](/en/servers/transforms/tool-transformation)** - Rename tools, modify descriptions, reshape arguments
- **[Enabled](/en/servers/visibility)** - Control which components are visible at runtime
- **[Tool Search](/en/servers/transforms/tool-search)** - Replace large tool catalogs with on-demand search
- **[Resources as Tools](/en/servers/transforms/resources-as-tools)** - Expose resources to tool-only clients
- **[Prompts as Tools](/en/servers/transforms/prompts-as-tools)** - Expose prompts to tool-only clients
- **[Code Mode (Experimental)](/en/servers/transforms/code-mode)** - Replace many tools with programmable `search` + `execute`
