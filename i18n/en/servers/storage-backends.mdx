...
The [OAuth Proxy](/en/servers/auth/oauth-proxy) and OAuth auth providers use storage for persisting OAuth client registrations and upstream tokens. **By default, storage is automatically encrypted using `FernetEncryptionWrapper`.** When providing custom storage, wrap it in `FernetEncryptionWrapper` to encrypt sensitive OAuth tokens at rest.
...
Both parameters are required for production. **Wrap your storage in `FernetEncryptionWrapper` to encrypt sensitive OAuth tokens at rest** - without it, tokens are stored in plaintext. See [OAuth Token Security](/en/deployment/http#oauth-token-security) and [Key and Storage Management](/en/servers/auth/oauth-proxy#key-and-storage-management) for complete setup details.

### Response Caching Middleware

The [Response Caching Middleware](/en/servers/middleware#caching-middleware) caches tool calls, resource reads, and prompt requests. Storage configuration is passed via the `cache_storage` parameter:
...
The [FastMCP Client](/en/clients/client) uses storage for persisting OAuth tokens locally. By default, tokens are stored in memory:
...
- [Response Caching Middleware](/en/servers/middleware#caching-middleware) - Using storage for caching
- [OAuth Token Security](/en/deployment/http#oauth-token-security) - Production OAuth configuration
- [HTTP Deployment](/en/deployment/http) - Complete deployment guide
