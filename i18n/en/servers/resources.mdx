...
<ParamField body="icons" type="list[Icon] | None">
  ...
  Optional list of icon representations for this resource or template. See [Icons](/en/servers/icons) for detailed examples
</ParamField>
...
<ParamField body="version" type="str | int | None">
  ...
  Optional version identifier for this resource. See [Versioning](/en/servers/versioning) for details.
</ParamField>
</Card>

#### Using with Methods

For decorating instance or class methods, use the standalone `@resource` decorator and register the bound method. See [Tools: Using with Methods](/en/servers/tools#using-with-methods) for the pattern.
...
Clients can handle these notifications using a [message handler](/en/clients/notifications) to automatically refresh their resource lists or update their interfaces.
...
Resources and resource templates support versioning, allowing you to maintain multiple implementations under the same URI while clients automatically receive the highest version. See [Versioning](/en/servers/versioning) for complete documentation on version comparison, retrieval, and migration patterns.