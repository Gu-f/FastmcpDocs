...
If low latency is a requirement for your use-case, consider using [`import_server()`](/en/servers/composition#importing-static-composition) to copy tools at startup rather than proxying them at runtime.
...
You can also pass a FastMCP [client transport](/en/clients/transports) (or parameter that can be inferred to a transport) to `as_proxy()`. This will automatically create a `ProxyClient` instance for you.
