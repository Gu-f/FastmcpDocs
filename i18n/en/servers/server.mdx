...
import { VersionBadge } from "/en/snippets/version-badge.mdx"
...
Each component type has detailed documentation: [Tools](/en/servers/tools), [Resources](/en/servers/resources) (including [Resource Templates](/en/servers/resources#resource-templates)), and [Prompts](/en/servers/prompts).
...
The server can also be run using the FastMCP CLI. For detailed information on transports and deployment, see [Running Your Server](/en/deployment/running-server).
...
<ParamField body="icons" type="list[Icon] | None">
  ...
  List of icon representations for your server. See [Icons](/en/servers/icons) for details
</ParamField>
...
<ParamField body="auth" type="OAuthProvider | TokenVerifier | None">
  Authentication provider for securing HTTP-based transports. See [Authentication](/en/servers/auth/authentication) for configuration
</ParamField>

<ParamField body="middleware" type="list[Middleware] | None">
  [Middleware](/en/servers/middleware) that intercepts and transforms every MCP message flowing through the server — requests, responses, and notifications in both directions. Use for cross-cutting concerns like logging, error handling, and rate limiting
</ParamField>

<ParamField body="providers" type="list[Provider] | None">
  [Providers](/en/servers/providers) that supply tools, resources, and prompts dynamically. Providers are queried at request time, so they can serve components from databases, APIs, or other external sources
</ParamField>

<ParamField body="transforms" type="list[Transform] | None">
  <VersionBadge version="3.1.0" />

  Server-level [transforms](/en/servers/transforms/transforms) to apply to all components. Transforms modify how tools, resources, and prompts are presented to clients — for example, [search transforms](/en/servers/transforms/tool-search) replace large catalogs with on-demand discovery
</ParamField>

<ParamField body="lifespan" type="Lifespan | AsyncContextManager | None">
  Server-level setup and teardown logic that runs when the server starts and stops. See [Lifespans](/en/servers/lifespan) for composable lifespans
</ParamField>
</Card>
...
<ParamField body="list_page_size" type="int | None" default="None">
  ...
  Maximum items per page for list operations (`tools/list`, `resources/list`, etc.). When `None`, all results are returned in a single response. See [Pagination](/en/servers/pagination) for details
</ParamField>
...
<ParamField body="sampling_handler" type="SamplingHandler | None">
  Custom handler for MCP sampling requests (server-initiated LLM calls). See [Sampling](/en/servers/sampling) for details
</ParamField>
...
Custom routes are useful for health checks, status endpoints, and simple webhooks. For more complex web applications, consider [mounting your MCP server into a FastAPI or Starlette app](/en/deployment/http#integration-with-web-frameworks).
