...
<ParamField body="icons" type="list[Icon] | None">
  ...
  List of icon representations for your server. Icons help users visually identify your server in client applications. See [Icons](/en/servers/icons) for detailed examples
</ParamField>

<ParamField body="auth" type="OAuthProvider | TokenVerifier | None">
  Authentication provider for securing HTTP-based transports. See [Authentication](/en/servers/auth/authentication) for configuration options
</ParamField>
...
See [Tools](/en/servers/tools) for detailed documentation.
...
See [Resources & Templates](/en/servers/resources) for detailed documentation.
...
See [Prompts](/en/servers/prompts) for detailed documentation.
...
For detailed information on each transport, how to configure them (host, port, paths), and when to use which, please refer to the [**Running Your FastMCP Server**](/en/deployment/running-server) guide.
...
For more complex web applications, consider [mounting your MCP server into a FastAPI or Starlette app](/en/deployment/http#integration-with-web-frameworks).
...
See the [Server Composition](/en/servers/composition) guide for full details, best practices, and examples.
...
See the [Proxying Servers](/en/servers/proxy) guide for details and advanced usage.
...
See the [FastAPI Integration](/en/integrations/fastapi) and [OpenAPI Integration](/en/integrations/openapi) guides for detailed examples and configuration options.
...
- **`strict_input_validation`**: Controls tool input validation mode (default: False for flexible coercion), set with `FASTMCP_STRICT_INPUT_VALIDATION`. See [Input Validation Modes](/en/servers/tools#input-validation-modes)
