...
<ParamField body="icons" type="list[Icon] | None">
  ...
  Optional list of icon representations for this prompt. See [Icons](/en/servers/icons) for detailed examples
</ParamField>
...
<ParamField body="version" type="str | int | None">
  ...
  Optional version identifier for this prompt. See [Versioning](/en/servers/versioning) for details.
</ParamField>
</Card>

#### Using with Methods

For decorating instance or class methods, use the standalone `@prompt` decorator and register the bound method. See [Tools: Using with Methods](/en/servers/tools#using-with-methods) for the pattern.
...
Clients can handle these notifications using a [message handler](/en/clients/notifications) to automatically refresh their prompt lists or update their interfaces.
...
Prompts support versioning, allowing you to maintain multiple implementations under the same name while clients automatically receive the highest version. See [Versioning](/en/servers/versioning) for complete documentation on version comparison, retrieval, and migration patterns.
