...
When using [mounted servers](/en/servers/composition), middleware behavior follows a clear hierarchy:
...
For HTTP-specific data (headers, client IP) when using HTTP transports, see [HTTP Requests](/en/servers/context#http-requests).
...
See [Storage Backends](/en/servers/storage-backends) for complete options.
...
For more complex tool transformations, consider [Transforms](/en/servers/transforms/transforms) instead.
...
See [Context State Management](/en/servers/context#state-management) for details.
