...
<ParamField body="icons" type="list[Icon] | None">
  ...
  Optional list of icon representations for this tool. See [Icons](/en/servers/icons) for detailed examples
</ParamField>
...
<ParamField body="version" type="str | int | None">
  ...
  Optional version identifier for this tool. See [Versioning](/en/servers/versioning) for details.
</ParamField>
...
See [Custom Dependencies](/en/servers/context#custom-dependencies) for more details on dependency injection.
...
See [Visibility](/en/servers/visibility) for the complete visibility control API including key formats, tag-based filtering, and provider-level control.
...
- **ChatGPT**: Skips confirmation prompts for read-only tools in Chat mode (see [ChatGPT integration](/en/integrations/chatgpt))
...
Clients can handle these notifications using a [message handler](/en/clients/notifications) to automatically refresh their tool lists or update their interfaces.
...
For full documentation on the Context object and all its capabilities, see the [Context documentation](/en/servers/context).
...
You can dynamically remove tools from a server through its [local provider](/en/servers/providers/local):
...
Tools support versioning, allowing you to maintain multiple implementations under the same name while clients automatically receive the highest version. See [Versioning](/en/servers/versioning) for complete documentation on version comparison, retrieval, and migration patterns.
