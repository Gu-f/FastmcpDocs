...
| **FastMCP Class** | `RemoteAuthProvider` | [`OAuthProxy`](/en/servers/auth/oauth-proxy) |

If your provider doesn't support DCR (most traditional OAuth providers), you'll need to use [`OAuth Proxy`](/en/servers/auth/oauth-proxy) instead, which bridges the gap between MCP's DCR expectations and fixed OAuth credentials.
...
When not set, `scopes_supported` defaults to the token verifier's `required_scopes`. For Azure AD specifically, see the [AzureJWTVerifier](/en/integrations/azure#token-verification-only-managed-identity) which handles this automatically.
...
→ **Complete WorkOS tutorial**: [AuthKit Integration Guide](/en/integrations/authkit)
