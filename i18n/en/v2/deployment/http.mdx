...
FastMCP supports multiple authentication methods to secure your remote server. See the [Authentication Overview](/en/v2/servers/auth/authentication) for complete configuration options including Bearer tokens, JWT, and OAuth.
...
For FastAPI-specific integration patterns including both mounting MCP servers into FastAPI apps and generating MCP servers from FastAPI apps, see the [FastAPI Integration guide](/en/v2/integrations/fastapi).
...
For more details on OAuth authentication, see the [Authentication guide](/en/v2/servers/auth).
...
By default, FastMCP's Streamable HTTP transport maintains server-side sessions. Sessions enable stateful MCP features like [elicitation](/en/v2/servers/elicitation) and [sampling](/en/v2/servers/sampling), where the server needs to maintain context across multiple requests from the same client.
...
If you're using the [OAuth Proxy](/en/v2/servers/auth/oauth-proxy), FastMCP issues its own JWT tokens to clients instead of forwarding upstream provider tokens. This maintains proper OAuth 2.0 token boundaries.
...
For more details on the token architecture and key management, see [OAuth Proxy Key and Storage Management](/en/v2/servers/auth/oauth-proxy#key-and-storage-management).
...
Once your server is deployed, you'll need to verify it's accessible and functioning correctly. For comprehensive testing strategies including connectivity tests, client testing, and authentication testing, see the [Testing Your Server](/en/v2/development/tests) guide.
