...
| **FastMCP Class** | `RemoteAuthProvider` | [`OAuthProxy`](/en/v2/servers/auth/oauth-proxy) |

If your provider doesn't support DCR (most traditional OAuth providers), you'll need to use [`OAuth Proxy`](/en/v2/servers/auth/oauth-proxy) instead, which bridges the gap between MCP's DCR expectations and fixed OAuth credentials.
...
→ **Complete WorkOS tutorial**: [AuthKit Integration Guide](/en/v2/integrations/authkit)
