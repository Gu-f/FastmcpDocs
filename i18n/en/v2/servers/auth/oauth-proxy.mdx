...
<Note>
  ...
  Proxy`](/en/v2/servers/auth/oidc-proxy) for automatic configuration. OIDC Proxy
...
<ParamField body="token_verifier" type="TokenVerifier" required>
  A [`TokenVerifier`](/en/v2/servers/auth/token-verification) instance to validate the
  provider's tokens
</ParamField>
...
<ParamField body="issuer_url" type="AnyHttpUrl | str | None">
  ...
  **When to set explicitly:**
  ...
  See the [HTTP Deployment guide](/en/v2/deployment/http#mounting-authenticated-servers) for complete mounting examples.
</ParamField>
...
For production deployments with multiple servers or cloud deployments, see [Storage Backends](/en/v2/servers/storage-backends) for available options.
...
<VersionBadge version="2.13.0" />
  ...
  **For production:**
  ...
  See [HTTP Deployment - OAuth Token Security](/en/v2/deployment/http#oauth-token-security) for complete production setup.
</ParamField>
...
See the [Token Verification guide](/en/v2/servers/auth/token-verification) for detailed setup instructions for your provider.
