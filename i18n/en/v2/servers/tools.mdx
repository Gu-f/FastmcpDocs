...
<ParamField body="icons" type="list[Icon] | None">
  ...
  Optional list of icon representations for this tool. See [Icons](/en/v2/servers/icons) for detailed examples
</ParamField>
...
See [Custom Dependencies](/en/v2/servers/context#custom-dependencies) for more details on dependency injection.
...
- **ChatGPT**: Skips confirmation prompts for read-only tools in Chat mode (see [ChatGPT integration](/en/v2/integrations/chatgpt))
...
Clients can handle these notifications using a [message handler](/en/v2/clients/messages) to automatically refresh their tool lists or update their interfaces.
...
For full documentation on the Context object and all its capabilities, see the [Context documentation](/en/v2/servers/context).
