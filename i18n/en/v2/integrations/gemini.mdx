...
In the above example, we connected to our local server using `stdio` transport. Because we're using a FastMCP client, you can also connect to any local or remote MCP server, using any [transport](/en/v2/clients/transports) or [auth](/en/v2/clients/auth) method supported by FastMCP, simply by changing the client configuration.
