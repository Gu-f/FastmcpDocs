...
A fair question. Any [Interactive Tool](/en/apps/prefab) can call a server tool — there's nothing stopping you from putting `CallTool("add_note")` inside a regular `@mcp.tool(app=True)`. It works for one or two tools. Things get harder once the app grows:
...
- **[Interactive Tools](/en/apps/prefab)** — the building blocks: charts, tables, dashboards, reactive state
- **[Examples](/en/apps/examples)** — complete working servers
- **[Development](/en/apps/development)** — preview and test app tools locally
