...
<Note>
When generating an MCP server from FastAPI, FastMCP uses OpenAPIProvider (v3.0.0+) under the hood to source tools from your FastAPI app's OpenAPI spec. See [Providers](/en/servers/providers/overview) to understand how FastMCP sources components.
</Note>
...
Because FastMCP's FastAPI integration is based on its [OpenAPI integration](/en/integrations/openapi), you can customize how endpoints are converted to MCP components in exactly the same way. For example, here we use a `RouteMap` to map all GET requests to MCP resources, and all POST/PUT/DELETE requests to MCP tools:
...
<Tip>
To learn more about customizing the conversion process, see the [OpenAPI Integration guide](/en/integrations/openapi).
</Tip>
...
If you're mounting an authenticated MCP server under a path prefix, see [Mounting Authenticated Servers](/en/deployment/http#mounting-authenticated-servers) for important OAuth routing considerations.
...
For more details on configuration options, see the [OpenAPI Integration guide](/en/integrations/openapi).