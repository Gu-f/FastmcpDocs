...
In the above example, we connected to our local server using `stdio` transport. Because we're using a FastMCP client, you can also connect to any local or remote MCP server, using any [transport](/en/clients/transports) or [auth](/en/clients/auth) method supported by FastMCP, simply by changing the client configuration.
